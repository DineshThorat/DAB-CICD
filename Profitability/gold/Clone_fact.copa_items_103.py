...
display(df_tme.filter(col("fisc_per")=='2024010').groupBy("fisc_per","bus_type_ext").agg(count("*")))

# COMMAND ----------

# MAGIC %sql
# MAGIC select distinct(bus_type_ext) from dev_gold.profitability.vol_mix_salesgrpcustmat

# COMMAND ----------

display(df_vol_mix_salesgrpcustmat.filter(col("fisc_per")=='2024010').groupBy("fisc_per","bus_type_ext").agg(count("*")))
...
df_gl_balances_test = df_gl_balances.filter(
...
df_joined = df_gl_balances_test.alias('glb') \
    .join(df_pc_mapping.alias('pcm'), df_gl_balances_test.profit_center == df_pc_mapping.profit_center, how='left_outer')
...
df_tme = df_joined.groupBy('fisc_per', 'pcm.bus_type_ext') \
