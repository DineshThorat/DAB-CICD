...
df_final_0 = df_result_custmat\
    .join(df_std_cogs, [(df_result_custmat.material == df_std_cogs.material) & (df_result_custmat.fisc_year == df_std_cogs.fisc_year)] ,how='left_outer').drop(df_std_cogs["material"], df_std_cogs["fisc_year"])

## FISC PERIOD ##
df_final_1 =  df_final_0.join(df_per.alias("fp"), df_final_0["fisc_per"] == df_per["FiscalPeriod"], how="left_outer")\
    .filter((~df_final_0['material'].isin('109580')) & (df_final_0["__START_AT"] <= df_per["PeriodEnd"]) & ((df_final_0["__END_AT"] <= df_per["PeriodEnd"]) | (df_final_0["__END_AT"].isNull()))) \
    ...
    df_final_3 = df_final_2.join(
        ...
        (
            ...
            (
                ...
                (df_final_2["fisc_per"] == df_ylp2["fisc_per"]) &
                (df_final_2["ref_docnum"] == df_ylp2["bill_docnum"]) &
                (df_final_2["material"] == df_ylp2["material"]) &
                (df_final_2["customer"] == df_ylp2["outlet"]) &
                (df_final_2["sales_docnum"] == df_ylp2["sales_docnum"]) &
                (df_final_2["plant"] == df_ylp2["plant"]) &
                (df_final_2["profit_center"] == df_ylp2["profit_center"])
            )
            |
            ...
            (
                ...
                (df_final_2["plant"] == df_ylp2["plant"]) &
                (df_final_2["profit_center"] == df_ylp2["profit_center"]) &
                (df_final_2["sales_itemnum"] == df_ylp2["sales_itemnum"])
            )
        ),
        how="left_outer"
    ).drop(
...
df_final.groupBy('fisc_per').agg(sum('excise_from_copa'),sum('base_price_adj')).display()
...
df_final = df_final.withColumn("fixed_disc_oninv", col("total_amount").cast('decimal(38,18)')) \
                   ...
                   .withColumn("promo_disc_oninv", (col("sum_trade_promo") * (-1)).cast('decimal(25,3)')) \
...
df_final = df_final.withColumn("dfr_cr_dr_memos", ((col("line_vol_mix")*col("total_cr_dr_amnt_hist"))+(col("mix_cust_total_uc")*col("line_vol_mix")*col("total_cr_dr_amnt_cur"))).cast('decimal(38,18)')).na.fill(0.00) \
                   ...
                   .withColumn("dfr_sales_curve", (col('line_vol_mix')*col('dfr_curve_amnt')*col("mix_total_uc")).cast('decimal(38,18)')) \
...
df_final = df_final\
                    ...
                    .withColumn("nsr_base_incidence",when(col("FISC_PER") < '2024001',col("gross_revenue") - col("excise_from_cogs") - (0.093 * (col("gross_revenue") - col("excise_from_cogs"))) - col("nka_discount")) 
                    .otherwise(col("gross_revenue") - col("excise_from_cogs") - when(col("total_disc_oninv") < 0, -col("total_disc_oninv")).otherwise(col("total_disc_oninv")) - when(col("dfr_fixed_disc") < 0, -col("dfr_fixed_disc")).otherwise(col("dfr_fixed_disc")) - when(col("dfr_variable_disc") < 0, -col("dfr_variable_disc")).otherwise(col("dfr_variable_disc")) - when(col("dfr_pickup_disc") < 0, -col("dfr_pickup_disc")).otherwise(col("dfr_pickup_disc"))- when(col("dfr_promo_disc") < 0, -col("dfr_promo_disc")).otherwise(col("dfr_promo_disc"))).cast('decimal(38,18)'))\
...
df_final = df_final.withColumn("haulage_std", (col("mktg_vol_pc")*col("haulage_cost_pc")).cast('decimal(38,18)'))\
                    ...
                    .withColumn("total_cogs", (col("cogs_concentrate")+col("cogs_commodities")+col("excise_from_cogs")+col("cogs_conversion")+col("cogs_under_over")+col("haulage_total")))
...
df_final = df_final.withColumn("delivery_std", (col("mktg_vol_pc")*col("delivery_cost_pc"))) \
                   ...
                   .withColumn("delivery_total", (col("total_costs_dlv")*col("delivery_mix")))\
...
df_final = df_final.withColumn("warehouse_std", (col("mktg_vol_pc")*col("warehouse_cost_pc"))) \
                   ...
                   .withColumn("warehouse_total", (col("total_costs_warehouse")*col("warehouse_mix")))\
...
# Step2 
df_tme_alloc = df_vol_mix_salesgrpcustmat.join(df_tme, on=["fisc_per","bus_type_ext"], how="left_outer").withColumn("tme_cost_alloc",  when(col("bus_type_ext") != 'xx', col("tme_cost") * col("mix_gtmt_mktg_uc") ) .otherwise(col("tme_cost") * col("mix_total_mktg_uc")) 
)

# Step3
df_final = df_final.join(df_tme_alloc, on=["fisc_per", "customer", "material"], how="left_outer").withColumn("tme", col("tme_cost_alloc") * col("line_vol_mix"))
df_final.display()
...
.withColumn("tme_curve_cost_alloc", col("tme_curve_gtmt") * col("mix_gtmt_mktg_uc"))
...
.withColumn("tme_curve", col("tme_curve_cost_alloc") * col("line_vol_mix"))
...
df_final = df_final.join(df_sellexp_alloc, on=["fisc_per", "customer", "material"], how="left_outer") \
    .withColumn("selling_expenses", col("sellexp_alloc") * col("line_vol_mix"))
...
# DBTITLE 1,df_budget_vol_mix
# Load the DataFrame and filter based on fiscal period
df_bdg = spark.table("dev_silver.files.budget_vol_nsr").filter(
    (col("fisc_per") >= v_start_fisc_per) & (col("fisc_per") <= v_end_fisc_per)
)
df_bdg.display()
# Perform the necessary aggregation
df_aggregated = df_bdg.groupBy(
    "fisc_per", 
    "bus_type_ext", 
    "trade_group", 
    "sales_group"
).agg(
    sum("mgsv_ucs").alias("marketing_vol_uc")
)
df_aggregated.display()
# Define the window specifications
window_fisc_per = Window.partitionBy("fisc_per")
window_trade_group = Window.partitionBy("fisc_per", "trade_group")
window_bus_type_ext = Window.partitionBy("fisc_per", "bus_type_ext")

# Apply the window functions for total marketing volume and per group volume
df_with_window = df_aggregated.withColumn(
    "total_mktg_vol_uc", sum("marketing_vol_uc").over(window_fisc_per)
).withColumn(
    "tradegrp_mktg_vol_uc", sum("marketing_vol_uc").over(window_trade_group)
).withColumn(
    "gtmt_mktg_vol_uc", sum("marketing_vol_uc").over(window_bus_type_ext)
)

# Calculate the mix ratios based on the windowed total volumes
df_budget_vol_mix = df_with_window.withColumn(
    "mix_total_mktg_uc", col("marketing_vol_uc") / col("total_mktg_vol_uc")
).withColumn(
    "mix_tradegrp_mktg_uc", col("marketing_vol_uc") / col("tradegrp_mktg_vol_uc")
).withColumn(
    "mix_gtmt_mktg_uc", col("marketing_vol_uc") / col("gtmt_mktg_vol_uc")
)

# Show the final result
df_budget_vol_mix= df_budget_vol_mix.select(
    "fisc_per",
    "bus_type_ext",
    "trade_group",
    "sales_group",  
    "marketing_vol_uc",
    "mix_total_mktg_uc",
    "mix_tradegrp_mktg_uc",
    "mix_gtmt_mktg_uc"
)
df_budget_vol_mix.display()

# COMMAND ----------

# DBTITLE 1,Support Services G&A
# Perform the join operations using spark.table for cc_mapping and gl_mapping
df_tech = df_gl_balances.alias("glb") \
    .join(spark.table("dev_silver.sharepoint.gl_mapping").alias("glm"), 
          on= col('glb.gl_account') == col('glm.gl_account'), 
          how='left_outer') \
    .join(spark.table("dev_silver.sharepoint.cc_mapping").alias("ccm"), 
          on= col('glb.cost_center') == col('ccm.cost_center'), 
          how='left_outer') \
    .filter((col('glm.pnl_level1_class') == 'Operating Expenses') & 
            (col('ccm.profit_tool3') == 'Technology') & 
            (col('glb.cdo') == 'no')) \
    .groupBy('glb.fisc_per') \
    .agg(sum('glb.balance').alias('tech_total'))


# Step 2
df_tech_sg = df_tech.join(df_budget_vol_mix, on=["fisc_per"], how="inner") \
    .withColumn("tech_sg", col("tech_total") * col("mix_total_mktg_uc"))

# Step 3
# Join df_tech_sg with df_vol_mix_salesgrpcustmat to allocate down to customer material level
df_tech_custmat = df_tech_sg.join(df_vol_mix_salesgrpcustmat, (df_tech_sg.fisc_per == df_vol_mix_salesgrpcustmat.fisc_per) & (df_tech_sg.sales_group == df_vol_mix_salesgrpcustmat.sales_grp), how="left_outer") \
    .withColumn("tech_custmat", col("tech_sg") * col("mix_salesgrp_mktg_uc")).drop(df_vol_mix_salesgrpcustmat.fisc_per)

# Step 4
# Join df_tech_custmat with df_final to allocate down to billing document line level
df_final = df_final.join(df_tech_custmat, on=["fisc_per", "customer", "material"], how="left_outer") 
...
# DBTITLE 1,People
# Step 1: Calculate the total value for People from GL
df_people = df_gl_balances.alias("glb") \
    .join(spark.table("dev_silver.sharepoint.gl_mapping").alias("glm"), 
          on=col("glb.gl_account") == col("glm.gl_account"), 
          how="left_outer") \
    .join(spark.table("dev_silver.sharepoint.cc_mapping").alias("ccm"), 
          ...
          how="left_outer") \
    .filter((col("glm.pnl_level1_class") == "Operating Expenses") & 
            (col("ccm.profit_tool3") == "People") & 
            (col("glb.cdo") == "no")) \
    .groupBy("glb.fisc_per") \
    .agg(sum("glb.balance").alias("people_total"))

# Step 2: Join with df_budget_vol_mix to allocate value down to sales group level
df_people_sg = df_people.join(df_budget_vol_mix, on=["fisc_per"], how="inner") \
    .withColumn("people_sg", col("people_total") * col("mix_total_mktg_uc"))
df_people_sg.display()
df_vol_mix_salesgrpcustmat.display()
# Step 3: Join with df_vol_mix_salesgrpcustmat to allocate down to customer material level
df_people_custmat = df_people_sg.join(df_vol_mix_salesgrpcustmat, (df_people_custmat.fisc_per == df_vol_mix_salesgrpcustmat.fisc_per) & (df_people_custmat.sales_group == df_vol_mix_salesgrpcustmat.sales_grp), how="left_outer") \
    .withColumn("people_custmat", col("people_sg") * col("mix_salesgrp_mktg_uc")).drop(df_vol_mix_salesgrpcustmat.fisc_per)

# Step 4: Join with df_final to allocate down to billing document line level
df_final = df_final.join(df_people_custmat, on=["fisc_per", "customer", "material"], how="left_outer") \
    .withColumn("people_indirect_exp", col("people_custmat") * col("line_vol_mix"))
...
# DBTITLE 1,Finance
# Step 1: Calculate the total value for Finance from GL
df_finance = df_gl_balances.alias("glb") \
    .join(spark.table("dev_silver.sharepoint.gl_mapping").alias("glm"), 
          on=col("glb.gl_account") == col("glm.gl_account"), 
          how="left_outer") \
    .join(spark.table("dev_silver.sharepoint.cc_mapping").alias("ccm"), 
          ...
          how="left_outer") \
    .filter((col("glm.pnl_level1_class") == "Operating Expenses") & 
            (col("ccm.profit_tool3") == "Finance") & 
            (col("glb.cdo") == "no")) \
    .groupBy("glb.fisc_per") \
    .agg(sum("glb.balance").alias("finance_total"))

# Step 2: Join with df_budget_vol_mix to allocate value down to sales group level
df_finance_sg = df_finance.join(df_budget_vol_mix, on=["fisc_per"], how="inner") \
    .withColumn("finance_sg", col("finance_total") * col("mix_total_mktg_uc"))

# Step 3: Join with df_vol_mix_salesgrpcustmat to allocate down to customer material level
df_finance_custmat = df_finance_sg.join(df_vol_mix_salesgrpcustmat, on=["fisc_per", "sales_grp"], how="left_outer") \
    .withColumn("finance_custmat", col("finance_sg") * col("mix_salesgrp_mktg_uc"))

# Step 4: Join with df_final to allocate down to billing document line level
df_final = df_final.join(df_finance_custmat, on=["fisc_per", "customer", "material"], how="left_outer") \
    .withColumn("finance_indirect_exp", col("finance_custmat") * col("line_vol_mix"))
...
# DBTITLE 1,Legal
# Step 1: Calculate the total value for Legal from GL
df_legal = df_gl_balances.alias("glb") \
    .join(spark.table("dev_silver.sharepoint.gl_mapping").alias("glm"), 
          on=col("glb.gl_account") == col("glm.gl_account"), 
          how="left_outer") \
    .join(spark.table("dev_silver.sharepoint.cc_mapping").alias("ccm"), 
          ...
          how="left_outer") \
    .filter((col("glm.pnl_level1_class") == "Operating Expenses") & 
            (col("ccm.profit_tool3") == "Legal") & 
            (col("glb.cdo") == "no")) \
    .groupBy("glb.fisc_per") \
    .agg(sum("glb.balance").alias("legal_total"))

# Step 2: Join with df_budget_vol_mix to allocate value down to sales group level
df_legal_sg = df_legal.join(df_budget_vol_mix, on=["fisc_per"], how="inner") \
    .withColumn("legal_sg", col("legal_total") * col("mix_total_mktg_uc"))

# Step 3: Join with df_vol_mix_salesgrpcustmat to allocate down to customer material level
df_legal_custmat = df_legal_sg.join(df_vol_mix_salesgrpcustmat, on=["fisc_per", "sales_grp"], how="left_outer") \
    .withColumn("legal_custmat", col("legal_sg") * col("mix_salesgrp_mktg_uc"))

# Step 4: Join with df_final to allocate down to billing document line level
df_final = df_final.join(df_legal_custmat, on=["fisc_per", "customer", "material"], how="left_outer") \
    .withColumn("legal_indirect_exp", col("legal_custmat") * col("line_vol_mix"))
...
# DBTITLE 1,Corporate & Regulatory Affairs
# Step 1: Calculate the total value for Corporate & Regulatory Affairs from GL
df_corp_reg = df_gl_balances.alias("glb") \
    .join(spark.table("dev_silver.sharepoint.gl_mapping").alias("glm"), 
          on=col("glb.gl_account") == col("glm.gl_account"), 
          how="left_outer") \
    .join(spark.table("dev_silver.sharepoint.cc_mapping").alias("ccm"), 
          ...
          how="left_outer") \
    .filter((col("glm.pnl_level1_class") == "Operating Expenses") & 
            (col("ccm.profit_tool3") == "Corporate & Regulatory Affairs") & 
            (col("glb.cdo") == "no")) \
    .groupBy("glb.fisc_per") \
    .agg(sum("glb.balance").alias("corp_reg_total"))

# Step 2: Join with df_budget_vol_mix to allocate value down to sales group level
df_corp_reg_sg = df_corp_reg.join(df_budget_vol_mix, on=["fisc_per"], how="inner") \
    .withColumn("corp_reg_sg", col("corp_reg_total") * col("mix_total_mktg_uc"))

# Step 3: Join with df_vol_mix_salesgrpcustmat to allocate down to customer material level
df_corp_reg_custmat = df_corp_reg_sg.join(df_vol_mix_salesgrpcustmat, on=["fisc_per", "sales_grp"], how="left_outer") \
    .withColumn("corp_reg_custmat", col("corp_reg_sg") * col("mix_salesgrp_mktg_uc"))

# Step 4: Join with df_final to allocate down to billing document line level
df_final = df_final.join(df_corp_reg_custmat, on=["fisc_per", "customer", "material"], how="left_outer") \
    .withColumn("corp_reg_indirect_exp", col("corp_reg_custmat") * col("line_vol_mix"))
...
# DBTITLE 1,General Management
# Step 1: Calculate the total value for General Management from GL
df_gen_mgmt = df_gl_balances.alias("glb") \
    .join(spark.table("dev_silver.sharepoint.gl_mapping").alias("glm"), 
          on=col("glb.gl_account") == col("glm.gl_account"), 
          how="left_outer") \
    .join(spark.table("dev_silver.sharepoint.cc_mapping").alias("ccm"), 
          ...
          how="left_outer") \
    .filter((col("glm.pnl_level1_class") == "Operating Expenses") & 
            (col("ccm.profit_tool3") == "General Management") & 
            (col("glb.cdo") == "no")) \
    .groupBy("glb.fisc_per") \
    .agg(sum("glb.balance").alias("gen_mgmt_total"))

# Step 2: Join with df_budget_vol_mix to allocate value down to sales group level
df_gen_mgmt_sg = df_gen_mgmt.join(df_budget_vol_mix, on=["fisc_per"], how="inner") \
    .withColumn("gen_mgmt_sg", col("gen_mgmt_total") * col("mix_total_mktg_uc"))

# Step 3: Join with df_vol_mix_salesgrpcustmat to allocate down to customer material level
df_gen_mgmt_custmat = df_gen_mgmt_sg.join(df_vol_mix_salesgrpcustmat, on=["fisc_per", "sales_grp"], how="left_outer") \
    .withColumn("gen_mgmt_custmat", col("gen_mgmt_sg") * col("mix_salesgrp_mktg_uc"))

# Step 4: Join with df_final to allocate down to billing document line level
df_final = df_final.join(df_gen_mgmt_custmat, on=["fisc_per", "customer", "material"], how="left_outer") \
    .withColumn("gen_mgmt_indirect_exp", col("gen_mgmt_custmat") * col("line_vol_mix"))
...
# DBTITLE 1,Strategic Planning
# Step 1: Calculate the total value for Strategic Planning from GL
df_strategic_planning = df_gl_balances.alias("glb") \
    .join(spark.table("dev_silver.sharepoint.gl_mapping").alias("glm"), 
          on=col("glb.gl_account") == col("glm.gl_account"), 
          how="left_outer") \
    .join(spark.table("dev_silver.sharepoint.cc_mapping").alias("ccm"), 
          ...
          how="left_outer") \
    .filter((col("glm.pnl_level1_class") == "Operating Expenses") & 
            (col("ccm.profit_tool3") == "Strategic Planning") & 
            (col("glb.cdo") == "no")) \
    .groupBy("glb.fisc_per") \
    .agg(sum("glb.balance").alias("strategic_planning_total"))

# Step 2: Join with df_budget_vol_mix to allocate value down to sales group level
df_strategic_planning_sg = df_strategic_planning.join(df_budget_vol_mix, on=["fisc_per"], how="inner") \
    .withColumn("strategic_planning_sg", col("strategic_planning_total") * col("mix_total_mktg_uc"))

# Step 3: Join with df_vol_mix_salesgrpcustmat to allocate down to customer material level
df_strategic_planning_custmat = df_strategic_planning_sg.join(df_vol_mix_salesgrpcustmat, on=["fisc_per", "sales_grp"], how="left_outer") \
    .withColumn("strategic_planning_custmat", col("strategic_planning_sg") * col("mix_salesgrp_mktg_uc"))

# Step 4: Join with df_final to allocate down to billing document line level
df_final = df_final.join(df_strategic_planning_custmat, on=["fisc_per", "customer", "material"], how="left_outer") \
    .withColumn("strategic_planning_indirect_exp", col("strategic_planning_custmat") * col("line_vol_mix"))
...
# DBTITLE 1,Special Admin
# Step 1: Calculate the total value for Special Admin from GL
df_special_admin = df_gl_balances.alias("glb") \
    .join(spark.table("dev_silver.sharepoint.gl_mapping").alias("glm"), 
          on=col("glb.gl_account") == col("glm.gl_account"), 
          how="left_outer") \
    .join(spark.table("dev_silver.sharepoint.cc_mapping").alias("ccm"), 
          ...
          how="left_outer") \
    .filter((col("glm.pnl_level1_class") == "Operating Expenses") & 
            (col("ccm.profit_tool3") == "Special Admin") & 
            (col("glb.cdo") == "no")) \
    .groupBy("glb.fisc_per") \
    .agg(sum("glb.balance").alias("special_admin_total"))

# Step 2: Join with df_budget_vol_mix to allocate value down to sales group level
df_special_admin_sg = df_special_admin.join(df_budget_vol_mix, on=["fisc_per"], how="inner") \
    .withColumn("special_admin_sg", col("special_admin_total") * col("mix_total_mktg_uc"))

# Step 3: Join with df_vol_mix_salesgrpcustmat to allocate down to customer material level
df_special_admin_custmat = df_special_admin_sg.join(df_vol_mix_salesgrpcustmat, on=["fisc_per", "sales_grp"], how="left_outer") \
    .withColumn("special_admin_custmat", col("special_admin_sg") * col("mix_salesgrp_mktg_uc"))

# Step 4: Join with df_final to allocate down to billing document line level
df_final = df_final.join(df_special_admin_custmat, on=["fisc_per", "customer", "material"], how="left_outer") \
    .withColumn("special_admin_indirect_exp", col("special_admin_custmat") * col("line_vol_mix"))
...
# DBTITLE 1,Other Subs
# Step 1: Calculate the total value for Other Subs from GL
df_other_subs = df_gl_balances.alias("glb") \
    .join(spark.table("dev_silver.sharepoint.gl_mapping").alias("glm"), 
          on=col("glb.gl_account") == col("glm.gl_account"), 
          how="left_outer") \
    .join(spark.table("dev_silver.sharepoint.cc_mapping").alias("ccm"), 
          ...
          how="left_outer") \
    .filter((col("glm.pnl_level1_class") == "Operating Expenses") & 
            (col("ccm.profit_tool2") == "Other Subs") & 
            (col("glb.cdo") == "no")) \
    .groupBy("glb.fisc_per") \
    .agg(sum("glb.balance").alias("other_subs_total"))

# Step 2: Join with df_budget_vol_mix to allocate value down to sales group level
df_other_subs_sg = df_other_subs.join(df_budget_vol_mix, on=["fisc_per"], how="inner") \
    .withColumn("other_subs_sg", col("other_subs_total") * col("mix_total_mktg_uc"))

# Step 3: Join with df_vol_mix_salesgrpcustmat to allocate down to customer material level
df_other_subs_custmat = df_other_subs_sg.join(df_vol_mix_salesgrpcustmat, on=["fisc_per", "sales_grp"], how="left_outer") \
    .withColumn("other_subs_custmat", col("other_subs_sg") * col("mix_salesgrp_mktg_uc"))

# Step 4: Join with df_final to allocate down to billing document line level
df_final = df_final.join(df_other_subs_custmat, on=["fisc_per", "customer", "material"], how="left_outer") \
    .withColumn("other_subs_indirect_exp", col("other_subs_custmat") * col("line_vol_mix"))
...
# DBTITLE 1,corporate_functions
# Create the corporate_functions column by adding all the required columns
df_final = df_final.withColumn(
    "corporate_functions",
    col("technology_indirect_exp") + 
    col("people_indirect_exp") + 
    col("finance_indirect_exp") + 
    col("legal_indirect_exp") + 
    col("corp_reg_indirect_exp") + 
    col("gen_mgmt_indirect_exp") + 
    col("strategic_planning_indirect_exp") + 
    col("special_admin_indirect_exp")
).withColumn(
    "Support_Services",
    col("corporate_functions") + col("other_subs_indirect_exp")
)
...
# MAGIC %md
# MAGIC ## Selling Expenses

# COMMAND ----------

df_cc_mapping = spark.table("dev_silver.sharepoint.cc_mapping")

df_joined_sell_exp = (
    df_gl_balances.alias("glb")
    .join(df_cc_mapping.alias("ccm"), df_gl_balances["cost_center"] == df_cc_mapping["cost_center"], how="left_outer")
    .join(df_pc_mapping, df_gl_balances["profit_center"] == df_pc_mapping["profit_center"], how="left_outer")
)

df_filtered_sell_exp = (
    df_joined_sell_exp
    .withColumn(
        "bus_type_ext",
        when(col(df_gl_balances.cost_center).isNull(), col(df_pc_mapping.bus_type_ext)).otherwise(col(df_cc_mapping.bus_type_ext))
    )
    .withColumn(
        "trade_grp",
        when(col(df_gl_balances.cost_center).isNull(), col(df_pc_mapping.trade_grp)).otherwise(col(df_cc_mapping.trade_grp))
    )
    .withColumn(
        "sales_grp",
        when(col(df_gl_balances.cost_center).isNull(), col(df_pc_mapping.sales_grp)).otherwise(col(df_cc_mapping.sales_grp))
    )
)
# display(df_filtered_sell_exp)
...
# select fisc_per,sum(ipp_discount) from dev_gold.fact.copa_items group by fisc_per
